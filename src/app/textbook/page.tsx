...
const chapterColors: Record<string, { bg: string; border: string; text: string; gradientBg: string; lightBg: string }> = {
  amber: {
    bg: "bg-amber-50",
...
function SubsectionBlock({
  ...
  return (
    <div className="mb-4">
      <h4 className={`font-bold text-sm ${color.text} mb-1`}>
        {sub.title}
      </h4>
      {sub.description && (
      ...
      {sub.sceneExample && (
        <div className={`${color.bg} border ${color.border} rounded-lg p-3 mt-2`}>
          <p className="text-xs text-gray-500 font-bold mb-1">生活シーン例</p>
...
function SectionBlock({
  ...
  const is80PointSection = section.heading === "80点主義のすすめ";
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm p-5 mb-4">
      ...
      {is80PointSection && (
        <div className={`${color.bg} border-2 ${color.border} rounded-xl p-4 mb-4`}>
          <p className={`text-sm font-black ${color.text} text-center`}>
            100点を1日やることより、70点を30日続けること
          </p>
        </div>
      )}
      ...
      {section.sceneExample && (
        <div className={`${color.bg} border ${color.border} rounded-lg p-3 mb-4`}>
          <p className="text-xs text-gray-500 font-bold mb-1">生活シーン例</p>
          <p className="text-sm text-gray-700 leading-relaxed italic">
            {section.sceneExample}
          </p>
          ...
          {section.comboExamples.map((combo, i) => (
            <div key={i} className={`${color.bg} border ${color.border} rounded-xl p-4`}>
              <div className="flex items-center justify-between mb-2">
                <p className={`text-xs font-black ${color.text} uppercase tracking-wider`}>
                  {combo.label}
                </p>
                {combo.kcal && (
      ...
      {section.keyMessage && (
        <div className={`${color.bg} border-l-4 ${color.border} rounded-r-lg p-4`}>
...
function ExerciseCard({
  ...
  const color = chapterColors[colorKey];
  return (
    <div className={`${color.bg} border ${color.border} rounded-xl p-4 mb-3`}>
      <div className="flex items-center gap-2 mb-2">
        <div className={`w-6 h-6 rounded-full bg-gradient-to-br ${color.gradientBg} flex items-center justify-center`}>
          <svg viewBox="0 0 16 16" fill="none" stroke="white" strokeWidth="2" className="w-3 h-3">
            <path d="M2 8l4 4 8-8" />
          </svg>
        </div>
...
function ChapterBlock({ chapter }: { chapter: TextbookChapter }) {
  ...
  return (
    <section id={`chapter-${chapter.number}`} className="scroll-mt-20">
      <div
        className={`bg-gradient-to-r ${color.gradientBg} rounded-2xl px-5 py-4 mb-4`}
      >
      ...
      {chapter.sections.map((s, i) => {
        ...
        return <SectionBlock key={i} section={s} colorKey={chapter.color} />;
      })}
    </section>
  );
}
...
export default function TextbookPage() {
  return (
    <main className="min-h-screen bg-gray-50">
      <HamburgerMenu />
      <TextbookProgress totalChapters={textbookChapters.length} />
      ...
      <div className="max-w-md mx-auto px-4 py-6">
        ...
        <div className="bg-white rounded-2xl shadow-sm p-5 mb-6">
          ...
          <div className="flex flex-col gap-2">
            {textbookChapters.map((ch) => {
              const color = chapterColors[ch.color] ?? chapterColors.amber;
              return (
                <a
                  key={ch.number}
                ...
                >
                  <span
                    className={`w-7 h-7 rounded-lg bg-gradient-to-br ${color.gradientBg} text-white text-xs font-black flex items-center justify-center shrink-0`}
                  >
                    {ch.number}
                  </span>
        ...
        <div className="bg-white rounded-2xl shadow-sm p-5 mb-6">
          ...
          <div className="flex flex-col items-center gap-3">
            <div className="bg-gradient-to-r from-amber-400 to-orange-400 text-white text-xs font-bold px-6 py-2 rounded-full">
              理想の体型・健康
            </div>
            <div className="grid grid-cols-4 gap-2 w-full">
              {[
                { label: "食事", sub: "何を食べるか", color: "from-emerald-400 to-green-400" },
                { label: "運動", sub: "どう動くか", color: "from-violet-400 to-purple-400" },
                { label: "習慣", sub: "どう続けるか", color: "from-orange-400 to-amber-400" },
                { label: "メンタル", sub: "どう考えるか", color: "from-rose-400 to-pink-400" },
              ].map((p) => (
                <div
                  key={p.label}
        ...
        <div className="bg-gradient-to-r from-amber-50 to-orange-50 border border-amber-200 rounded-2xl p-5 mb-6 text-center">
          <p className="text-sm font-bold text-amber-700 leading-relaxed">
            正しいやり方で継続さえすれば、必ず求める成果を手に入れることができる
          </p>
        </div>
